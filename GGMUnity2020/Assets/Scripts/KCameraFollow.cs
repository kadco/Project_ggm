...
public class KCameraFollow : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.UpArrow))
            target.position += target.forward * 1.0f * Time.deltaTime;
        if (Input.GetKey(KeyCode.DownArrow))
            target.position += -target.forward * 1.0f * Time.deltaTime;

        //회전
        if (Input.GetKey(KeyCode.RightArrow))
            target.RotateAround(target.position, Vector3.up, 90 * Time.deltaTime);
    ...
    void LateUpdate()
    {
        ...
        tr.position = target.position 
            - (rot * Vector3.forward * dist)  //rot 반영안하면 고정 시점
            + (Vector3.up * height);
